...
QueueFamilyIndices QueueFamilyIndices::FindQueueFamilies(VkPhysicalDevice device)
{
    ...
    for (const auto& queueFamily : queueFamilies) {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(device, i, application.GetInstanceManager().GetSurface(), &presentSupport);
        if (presentSupport) {
...
// Params: NONE
// Return: NONE
void InstanceManager::InitWindow()
{
...
// Params: NONE
// Return: NONE
VkSampleCountFlagBits InstanceManager::GetMaxUsableSampleCount()
{
...
// Params: device
// Return: bool
int InstanceManager::RateSuitableDevices(VkPhysicalDevice device)
{
    int score = 0;
...
// Params: device
// Return: bool
bool InstanceManager::CheckDeviceExtensionSupport(VkPhysicalDevice device) {
...
SwapChainSupportDetails InstanceManager::QuerySwapChainSupport(VkPhysicalDevice device) {
...
void InstanceManager::CreateLogicalDevice()
{
    ...
    QueueFamilyIndices indices = QueueFamilyIndices::FindQueueFamilies(m_physicalDevice);
...
// Desc: Sets up the debug messenger, to output runtime errors Vulkan reports
// Params: NONE
// Return: NONE
...
// Params: createInfo
// Return: NONE
void InstanceManager::PopulateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& create_info) {
    create_info = {};
...
// Params: NONE
// Return: bool
bool InstanceManager::CheckValidationLayerSupport() const
{
    uint32_t layerCount;
...
// Params: createInfo
// Return: VkResult
VkResult InstanceManager::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* p_create_info,
...
// Desc: Destroy the created VkDebugUtilsMessengerCreateInfoEXT structs
// Params: createInfo
// Return: VkResult
void InstanceManager::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator) {
...
void RenderManager::CreateSwapChain() {
    ...
    createInfo.surface = instanceManager.GetSurface();
    ...
    QueueFamilyIndices indices = QueueFamilyIndices::FindQueueFamilies(instanceManager.GetPhysicalDevice());
...
void RenderManager::RecreateSwapChain()
{
    ...
    while (width == 0 || height == 0) {
        glfwGetFramebufferSize(instanceManager.GetWindow(), &width, &height);
        glfwWaitEvents();
    }

    vkDeviceWaitIdle(instanceManager.GetDevice());     // Wait until the resources are free to use

    m_oldSwapChain = m_swapChain;   // Set the current chain to the old one so we can keep rendering using it

    CleanUp();                      // Cleanup the current one
    ...
    imageManager.InitialiseImages();        // Directly depend on the swap chain
...
VkSurfaceFormatKHR RenderManager::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& available_formats)
{
...
// Return: VkSurfaceFormatKHR
VkPresentModeKHR RenderManager::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& available_present_modes)
{
...
// Desc: Sets up the graphics pipeline
// Params: NONE
// Return: NONE
...
void SyncObjectManager::CreateSyncObjects()
{
    ...
    m_imagesInFlight.resize(renderManager.GetSwapChainImageBuffers().size(), VK_NULL_HANDLE);
...
// =================================================
// Name: CleanUp
// Desc: Clean up the objects our sync manager made
// Params: NONE
// Return: NONE
...
void ImageManager::CreateFrameBuffers()
{
    ...
    for (size_t i = 0; i < renderManager.GetSwapChainImageBuffers().size(); i++) {
        ...
        frameBufferInfo.renderPass = renderManager.GetRenderPass();
        frameBufferInfo.attachmentCount = static_cast<uint32_t>(attachments.size());
        frameBufferInfo.pAttachments = attachments.data();
        frameBufferInfo.width = renderManager.GetSwapChainExtent().width;
...
void ImageManager::CreateImageSampler()
{
    ...
    vkGetPhysicalDeviceProperties(instanceManager.GetPhysicalDevice(), &properties);
    // And set it here
    samplerInfo.anisotropyEnable = instanceManager.m_AnisotropyEnabled; // If not supported toggle it off
...
void BufferManager::CleanUp()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vkDestroyBuffer(device, m_uniformBuffers[i].GetBuffer(), nullptr);
        vkFreeMemory(device, m_uniformBuffers[i].GetBufferMemory(), nullptr);
    }
}
// =================================================
...
void BufferManager::UpdateUniformBuffers(uint32_t currentImage)
{
    ...
    ubo.proj = glm::perspective(glm::radians(45.0f), static_cast<float>(renderManager.GetSwapChainExtent().width) / static_cast<float>(renderManager.GetSwapChainExtent().height), 0.1f, 10.0f);
    // Invert as glm was made for OpenGL
    ubo.proj[1][1] *= -1;

    // TODO: This is inefficient and would be better as a push constant
    void* data;
    vkMapMemory(instanceManager.GetDevice(), m_uniformBuffers[currentImage].GetBufferMemory(), 0, sizeof(ubo), 0, &data);
    memcpy(data, &ubo, sizeof(ubo));
    vkUnmapMemory(instanceManager.GetDevice(), m_uniformBuffers[currentImage].GetBufferMemory());
}
// =================================================
// Name: CreateDescriptorPool
// Desc: A pool for descriptors to be created from
// Params: NONE
// Return: NONE
...
// Desc: Create the descriptor sets from the command pool to describe our buffers
// Params: NONE
// Return: NONE
void BufferManager::CreateDescriptorSets()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        vkUpdateDescriptorSets(instanceManager.GetDevice(), static_cast<uint32_t>(writeDescriptors.size()), writeDescriptors.data(),
            0, nullptr);
    }
}
// =================================================
...
void BufferManager::RecordCommandBuffer(VkCommandBuffer buffer, uint32_t imageidx)
{
    ...
    renderPassInfo.renderPass = renderManager.GetRenderPass();
    renderPassInfo.framebuffer = renderManager.GetSwapChainFrameBuffers()[imageidx];

    // Define the size of the render area
    renderPassInfo.renderArea.offset = { 0, 0 };
    renderPassInfo.renderArea.extent = renderManager.GetSwapChainExtent();
    ...
    vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderManager.GetGraphicsPipeline());

    //VkBuffer vertexBuffers[] = { m_vertexBuffer }; // We only have one buffer
    std::vector<VkDeviceSize> offsets;              // And offset
    offsets.emplace_back(0);
    uint32_t maxIndices = application.m_models[0].GetVertexArray().size();

    for (uint32_t i = 1; i < application.m_models.size(); ++i)
    {
        offsets.emplace_back(offsets[i - 1] + sizeof(application.m_models[i].GetVertexBuffer()));
        maxIndices += application.m_models[i].GetVertexArray().size();

    }

    // Bind our vertex buffers to the bindings specified
    vkCmdBindVertexBuffers(buffer, 0, 1, &application.m_models.data()->GetVertexBuffer().GetBuffer(), offsets.data());

    vkCmdBindIndexBuffer(buffer, application.m_models.data()->GetIndexBuffer().GetBuffer(), 0, VK_INDEX_TYPE_UINT32);

    vkCmdBindDescriptorSets(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderManager.GetGraphicsPipelineLayout(), 0, 1, 
        &m_descriptorSets[application.GetCurrentFrame()], 0, nullptr);
...
void BufferManager::EndSingleTimeCommands(VkCommandBuffer cmdBuffer)
{
    ...
    vkFreeCommandBuffers(instanceManager.GetDevice(), instanceManager.GetCommandPool(), 1, &cmdBuffer);
}
...
void Texture::CreateTextureImage(const char* filePath, bool hasMipLevels)
{
    ...
    void* data;
    vkMapMemory(application.GetInstanceManager().GetDevice(), stagingMemory, 0, imgSize, 0, &data);
    memcpy(data, pixels, imgSize);
    vkUnmapMemory(application.GetInstanceManager().GetDevice(), stagingMemory);
    ...
    vkDestroyBuffer(application.GetInstanceManager().GetDevice(), stagingBuffer, nullptr);
    vkFreeMemory(application.GetInstanceManager().GetDevice(), stagingMemory, nullptr);
}
...
void HelloTriangleApplication::InitVulkan()
{
    ...
    //---- Instance ----
    m_instanceManager.CreateAppInstance();
    //---- Renderer ----
    m_renderManager.InitialiseRenderer();
    //---- Images ----
    m_imageManager.CreateImageSampler();
    m_imageManager.InitialiseImages();
    //---- Buffers ----
    Model vikingRoom = Model("Models/viking_room.txt", "Textures/viking_room.png", true);
    m_models.push_back(vikingRoom);
    m_bufferManager.CreateBuffers();
    //---- Sync Objects ----
    m_syncManager.CreateSyncObjects();
}
...
{
    while (!glfwWindowShouldClose(m_instanceManager.GetWindow())) {
        glfwPollEvents();
        DrawFrame();
    }

    // Wait for everything to finish before destroying
    vkDeviceWaitIdle(m_instanceManager.GetDevice());
}

void HelloTriangleApplication::DrawFrame()
{
    // Check if a previous frame is using this image (i.e. there is its fence to wait on)
	vkWaitForFences(m_instanceManager.GetDevice(), 1, &m_syncManager.GetInFlightFences()[m_currentFrame], VK_TRUE, UINT64_MAX);

    // Get the image from the swap chain we want to draw
    uint32_t imageIndex;
    VkResult result = vkAcquireNextImageKHR(m_instanceManager.GetDevice(), m_renderManager.GetSwapChain(), UINT64_MAX,
        m_syncManager.GetAvailableSemaphores()[m_currentFrame], VK_NULL_HANDLE, &imageIndex);

    // If the window no longer matches the swap chain recreate it
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        m_renderManager.RecreateSwapChain();
	    return;                 // and exit as we cannot proceed
    }
    vkResetFences(m_instanceManager.GetDevice(), 1, &m_syncManager.GetInFlightFences()[m_currentFrame]);

    m_bufferManager.UpdateUniformBuffers(m_currentFrame);

    vkResetCommandBuffer(m_renderManager.GetCommandBuffer()[m_currentFrame], 0);
    m_bufferManager.RecordCommandBuffer(m_renderManager.GetCommandBuffer()[m_currentFrame], imageIndex);

    // Queue submission and synchronization
    VkSubmitInfo submitInfo{};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;

    VkSemaphore waitSemaphores[] = { m_syncManager.GetAvailableSemaphores()[m_currentFrame] };
    ...
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &m_renderManager.GetCommandBuffer()[m_currentFrame];

    // Which semaphore to signal when execution is finished
    VkSemaphore signalSemaphores[] = { m_syncManager.GetFinishedSemaphores()[m_currentFrame] };
    ...
    VkSwapchainKHR swapChains[] = { m_renderManager.GetSwapChain() };
    ...
    result = vkQueuePresentKHR(m_instanceManager.GetPresentQueue(), &presentInfo);
